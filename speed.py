...
def _loop_and_detect(
    ...
    Thread-safe speed_route_queue operations with lock
    Prevent division by zero with max(1, W // 2)
    """
    full_scrn = False
    fps = 0.0
    tic = time.time()
    ...
    while True:
        ...
        if not using_ball_logic:
            ...
            # Use manda._speed_lock if available (from handler.py)
            if hasattr(manda, '_speed_lock'):
                ...
                # No lock available, direct access
                has_route = bool(manda.speed_route_queue)
                if has_route and manda.speed_active_wp is None and manda.speed_route_queue:
                    manda.speed_active_wp = manda.speed_route_queue[0]
            ...
            # If NO color route yet
                ...
                if manda.speed_first_branch is not None:
                    # Route already chosen but queue empty -> mission complete (safety net)
        ...
        rospy.logdebug_throttle(5.0,
            f"[SPEED] Greenlight: {'YES' if greenlight_ada else 'NO'}"
        ...
        # Route decision at WP_BRANCH_DECISION (e.g. WP2)
        curr_wp = int(getattr(manda, "current_wp_index", -1))
        if (curr_wp == WP_BRANCH_DECISION and
            ...
            if redlight_ada and not greenlight_ada:
                # Only RED = use red route
                manda.speed_first_branch = 3
                manda.speed_route_queue = SPEED_ROUTE_RED.copy()
                manda.speed_active_wp = None
                rospy.loginfo(f"[SPEED] Decision at WP{WP_BRANCH_DECISION}: RED -> route {SPEED_ROUTE_RED}")

            elif greenlight_ada and not redlight_ada:
                # Only GREEN = use green route
                manda.speed_first_branch = 4
                manda.speed_route_queue = SPEED_ROUTE_GREEN.copy()
                manda.speed_active_wp = None
                rospy.loginfo(f"[SPEED] Decision at WP{WP_BRANCH_DECISION}: GREEN -> route {SPEED_ROUTE_GREEN}")

            else:
                # FALLBACK: no light seen -> assume RED
                rospy.logwarn("[SPEED] WP2: no light detected, fallback to RED route")
                manda.speed_first_branch = 3
                manda.speed_route_queue = SPEED_ROUTE_RED.copy()
                manda.speed_active_wp = None

        # Done if reached target ONLY if
        # not using color route and never chose branch
        if (manda.has_reached_target()
        ...
        # If representative bbox exists, draw thick box / additional label
        if best_green is not None:
